...
# 텐서플로우 version 2
# 1. Graph 모드 : Graph 를 미리 다 build 해놓으면 나중에 계산이 쉬워짐. 속도가 빠름.
# 2. Eagerly 모드 : 계산을 eagerly 하게 수행할 수 있도록 도움.
# - tf1은 Graph 만 모드 가지고 있고, tf2는 Graph 와 Eagerly 다 같이 있음.
# - 컴파일된 그래프의 이점을 그대로 보존함.
# - Keras 를 딥러닝의 고수준 API 를 채택함.
# - 텐서플로우는 기본적으로 수학 계산 및 미분계수를 구해주는 도구임
